...
<body>
    <?php
        ...
        $numberOfpages = ceil($numOfResult / $pageNumber);

        // current visited page number
        if(!isset($_GET['page'])){
            $page = 1;
        } else {
            $page = $_GET['page'];
        ...
        $pageFirstResult = ($page-1) * $pageNumber; 


        // Create Query
        if (strlen($search) > 0){
            $query = "SELECT transaction.datelog, transaction.action, transaction.remarks, transaction.documentcode, office.name as office_name, CONCAT(employee.lastname, ', ', employee.firstname) as employee_name FROM employee, office, transaction WHERE transaction.employee_id = employee.id and transaction.office_id = office.id and transaction.documentcode=" . $search . " ORDER BY transaction.documentcode, transaction.datelog LIMIT " . $pageFirstResult . "," . $pageNumber;
        }else {
            $query = "SELECT transaction.datelog, transaction.action, transaction.remarks, transaction.documentcode, office.name as office_name, CONCAT(employee.lastname, ', ', employee.firstname) as employee_name FROM employee, office, transaction WHERE transaction.employee_id = employee.id and transaction.office_id = office.id LIMIT " . $pageFirstResult . "," . $pageNumber;
        }
                            ...
                            <br/>
                                <div class="col-md-12">
                                    <form action="transaction.php" method="GET">
                                        ...
                                        <button type="submit" value="Search" class="btn btn-fill btn-info">Search</button>
                                    </form>
                                </div>
                                <div class="col-md-12">
